...
def test_get_all_posts(authorized_client, test_posts):
  ...
  def validate(post): 
    return schemas.PostOut(**post)    #validating response with schema
...
def test_get_one_post_not_exist(authorized_client, test_posts):
  res = authorized_client.get(f"/posts/20983023")
...
@pytest.mark.parametrize("title, content, published", [
  ...
  ("favote pizza", "pizza is good", False),
  ("anotha one", "wahoo", True)
])
...
def test_delete_post_nonexist(authorized_client, test_posts, test_user):
  res = authorized_client.delete(f"/posts/23409820394")
  assert res.status_code == 404
...
def test_update_post_not_exist(authorized_client, test_user, test_posts):
  ...
  res = authorized_client.put(f"/posts/234234234", json=data)
  assert res.status_code == 404